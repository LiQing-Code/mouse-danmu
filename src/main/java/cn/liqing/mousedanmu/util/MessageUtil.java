...
public class MessageUtil {
    ...
    public static void sendActionBar(@Nullable PlayerEntity player, Text message) {
        ...
        try {
            if (useTwoParams) {
                // MC 1.21+: player.sendMessage(text, true)
                sendMessageMethod.invoke(player, message, true);
            } else {
                // MC 1.19.4-1.20.x: player.sendMessage(text, true)
                // 注意：某些旧版本可能不支持，降级为普通消息
                try {
                    Method actionBarMethod = PlayerEntity.class.getMethod("sendMessage", Text.class, boolean.class);
                    actionBarMethod.invoke(player, message, true);
                } catch (NoSuchMethodException e) {
                    // 降级为普通消息
